...
**Last completed:** Amélioration post-feature (2026-07-02) — Jalons projet enrichis (`responsibleUserId`, `status` MilestoneStatus PENDING/IN_PROGRESS/DONE/DELAYED) dans `MilestoneFormModal.tsx` + `ProjectMilestonesList.tsx` + `MilestoneStatusBadge.tsx`, `createMilestone` validé par Zod (`lib/schemas/project.ts` — `milestoneSchema`) ; Certificat des Résultats Clés SKILLS_DEVELOPMENT converti d'un upload S3 (`Attachment`) vers un simple lien externe (`KeyResult.certificateUrl`) — suppression de `uploadCertificate`/upload fichier dans `KeyResultCard.tsx`, ajout d'un champ URL dans `KeyResultUpdateModal.tsx`, simplification de `app/objectives/page.tsx` et `app/department-objectives/page.tsx` (plus de signature d'URL S3 par lot)
...
- **Reset complet de la base (2026-07-02)** : l'historique de migrations était en drift par rapport à la base réelle (plusieurs colonnes/enums existaient en base — `Role.INTERN`, `GanttTask.status`, `Committee.description`/`projectId`, `Department.parentDepartmentId`, `Project.categoryOther`, `ProjectExpense.expenseCategory`, contrainte unique `WeekPlanner` — sans migration correspondante committée). `prisma migrate dev` refusait d'avancer sans `migrate reset`. Sur consentement explicite du client, la base a été réinitialisée (`prisma migrate reset`, toutes les données perdues) puis le compte Administrateur (`mabibe.bankati@facamstairway.com`, même id, même hash de mot de passe, département "Direction Générale" recréé à l'identique) a été restauré manuellement. Toutes les autres données (projets, utilisateurs, comités, objectifs, etc.) sont reparties de zéro à cette date — la migration `20260702092050_milestone_responsible_status_kr_certificate_url` capture à la fois le rattrapage du drift et les nouveaux champs (jalons, `certificateUrl`).
